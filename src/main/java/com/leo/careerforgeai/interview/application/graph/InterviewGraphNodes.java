 ...
 * @description: 实现面试Graph的冻结上下文校验和首题生成持久化节点
 * @author: Miao Zheng
 * @date: 2026-08-28
 **/
@Component
...
public class InterviewGraphNodes {
    ...
    public InterviewGraphNodes(
        ...
        this.questionGenerationService = Objects.requireNonNull(
                questionGenerationService,
                "questionGenerationService不能为空"
        );
    ...
    public Map<String, Object> loadFrozenContext(InterviewGraphState state) {
        ...
        MockInterviewSession session = sessionRepository
                .findById(ownerId, state.interviewId())
        ...
        if (session.status() != InterviewStatus.CREATED
            ...
            throw new IllegalStateException("当前Session状态不能进入首题Graph");
        }
        return Map.of();
    }
    ...
    public Map<String, Object> generateAndPersistQuestion(InterviewGraphState state) {
        Objects.requireNonNull(state, "state不能为空");
        if (state.currentRound() < 0 || state.currentRound() > 1) {
            throw new IllegalStateException("检查点6只允许生成或恢复首题");
        }

        InterviewQuestion question = questionGenerationService
                .generateAndPersistFirstQuestion(state.interviewId(), modelCallTimeout);

        if (!question.interviewId().equals(state.interviewId())) {
            throw new IllegalStateException("生成的问题不属于当前面试");
        }
        return InterviewGraphState.waitingForAnswerUpdate(1, question.questionId());
    }
    ...
    public Map<String, Object> validateAnswerResume(InterviewGraphState state) {
        ...
        InterviewRound round = roundRepository
                .findRoundByNumber(ownerId, state.interviewId(), state.currentRound())
        ...
        InterviewQuestion question = roundRepository
                .findQuestionByRound(ownerId, state.interviewId(), round.roundId())
        ...
        InterviewAnswer answer = roundRepository
                .findAnswerByQuestion(ownerId, state.interviewId(), questionId)
